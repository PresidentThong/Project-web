...
function show_product(products) {
    ...
    for (var product of products) {
      ...
      <div class="col-6 col-sm-4 col-lg-3 mt-3 p-3 product">
        ...
        <div class="text-end">
         <button onclick="add_to_cart(this.id)" id="${product.id}" class="btn btn-danger btn-sm">Add to Cart</button>
        </div>
      </div>
  `;
    }
...
/*function add_to_cart(product_id) {
    ...
    db.transaction(function (tx) {
        ...
        tx.executeSql(
            ...
            function (tx, result) {
                if (result, row[0]) {
                    update_cart_database(product_id, result, row[0].quantity + 1);
                } else {
...
function update_cart_quantity() {
    ...
    db.transaction(function (tx) {
        ...
        tx.executeSql(query,
         ...
         function (tx, result) {
             if (result.row[0]) {
                var cart_quantity = document.getElementById("cart-quantity").innerText = 
                result.row[0].total_quantity;
             }
          }, transaction_error);
    });

    var cart_quantity = document.getElementById("cart-quantity");
    var quantity = parseInt(cart_quantity.innerText);
    cart_quantity.innerText = quantity + 1;
} */
...
function add_cart_database(product_id) {
    ...
    `;
        ...
        function logout() {
            ...
            document.getElementById("account-info").innerHTML = `
    <button type="submit" class="btn btn-outline-danger ms-3" data-bs-toggle="modal" data-bs-target="#frm-login">Login</button> 
    `;
        }
